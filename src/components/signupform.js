...
class Form extends React.Component {
  ...
  render() {
    ...
    return (
      <form name={ this.props.name ? this.props.name : 'newsletter' } onSubmit={ this.handleSubmit } className="mt-8 block w-full float-left mb-16 bg-blacker rounded-sm p-8 text-white" method="POST" data-netlify="true" netlify-honeypot="bot-field" netlifyid="mc-embedded-subscribe-form" action="/thankyou/">
        <strong className="text-xs uppercase tracking-wide text-white">{ title }</strong>
        <p className="leading-normal mt-4 pr-8">{ text }</p>
        <p className="hidden">
